...
(defclass root-directory ()
  ((root-node)
   (node-subspace :initarg :node-subspace)
   (content-subspace :initarg :content-subspace)
   (allocator)))
...
(defgeneric directory-subspace-exists-p (directory-subspace tr &optional sub-path)
  (:method (dir (db database) &optional sub-path)
           (with-transaction (tr db)
             (directory-subspace-exists-p tr sub-path)))
...
(defgeneric directory-subspace-open (directory-subspace tr
                                     &key sub-path if-exists if-does-not-exist layer prefix)
  (:method (dir (db database) &rest args)
           (with-transaction (tr db)
             (apply #'directory-subspace-open tr args)))
...
(defgeneric directory-subspace-rename (directory-subspace tr path1 &optional path2)
  (:method (dir (db database) path1 &optional path2)
           (with-transaction (tr db)
             (directory-subspace-rename tr path1 path2)))
...
(defgeneric directory-subspace-delete (directory-subspace tr
  ...
  (:method (dir (db database) &rest args)
           (with-transaction (tr db)
             (apply #'directory-subspace-delete tr args)))
  (:method ((dir directory-subspace) (tr transaction) &rest args &key sub-path &allow-other-keys)
           (with-slots (directory path) dir
...
(defgeneric directory-subspace-list (directory-subspace tr &optional sub-path)
  (:method (dir (db database) &optional sub-path)
           (with-transaction (tr db)
             (directory-subspace-list tr sub-path)))
  (:method ((dir directory-subspace) (tr transaction) &optional sub-path)
           (with-slots (directory path) dir
             (directory-subspace-list-internal directory (combine-paths path sub-path) tr)))
  (:method ((dir root-directory) (tr transaction) &optional sub-path)
           (directory-subspace-list-internal dir sub-path tr)))
  ...
  (let ((existing-node (find-node dir tr path)))
    (if existing-node
        (ecase if-exists
          ...
          ((nil)
           (let ((existing-layer (future-value 
                                  (transaction-get tr (subspace-encode-key existing-node *layer-key*)))))
        ...
        (ecase if-does-not-exist
          ...
          (:create
           ...
           (let* ((length (tuple-length path))
                  (parent-node (if (zerop length)
                                   (slot-value dir 'root-node)
                                   (node-with-prefix dir (tuple-subtuple path 0 (1- length)))))
                  (node (node-with-prefix dir prefix)))
             (transaction-set tr
                              (subspace (subspace parent-node *sub-dir-key*)
                                        (tuple-elt path (1- length)))
                              prefix)
             (when layer
...
(defun directory-subspace-rename-internal (dir old-path new-path tr)
  ...
  (let ((old-node (find-node dir tr old-path)))
    ...
    (let* ((length (tuple-length new-path))
           (parent-node (find-node dir tr (tuple-subtuple new-path 0 (1- length)))))
      (when (null parent-node)
        (error "Parent directory of ~S does not exist" new-path))
      (transaction-set tr (subspace parent-node (tuple-elt new-path (1- length)))
                       (contents-of-node dir old-node nil nil)))
    (remove-from-parent dir tr old-path)
    (contents-of-node dir old-node new-path (future-value (transaction-get tr (subspace old-node *layer-key*))))))
...
(defun contents-of-node (dir node path layer)
  (let ((prefix (tuple-elt (subspace-decode-key node (key-bytes node)) 0)))
    ;; TODO: What about content-subspace??
    (make-instance 'directory-subspace :directory dir :path path 
                                       :raw-prefix prefix :layer layer)))

(defun find-node (dir tr path)
  (let ((node (slot-value dir 'root-node)))
    (dotimes (i (tuple-length path))
      (let ((prefix (subspace-encode-key node (tuple-elt path i))))
...
(defun remove-from-parent (dir tr path)
  (let ((parent (find-node dir tr (tuple-subtuple path 0 (1- (tuple-length path))))))
    (transaction-clear tr (subspace parent (tuple-elt path (1- (tuple-length path)))))))
